...
from detectron2.layers import ShapeSpec, batched_nms, cat, cross_entropy, mae, nonzero_tuple
...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, regressions_per_image, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for scores_per_image, boxes_per_image, regressions_per_image, image_shape in zip(scores, boxes, regressions, image_shapes)
    ]
...
def fast_rcnn_inference_single_image(
    ...
    if not valid_mask.all():
        ...
        regressions = regressions[valid_mask]
    ...
    regressions = regressions[filter_mask]

    # 2. Apply NMS for each class independently.
    keep = batched_nms(boxes, scores, filter_inds[:, 1], nms_thresh)
    if topk_per_image >= 0:
        keep = keep[:topk_per_image]
    boxes, scores, regressions, filter_inds = boxes[keep], scores[keep], regressions[keep], filter_inds[keep]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.regressions = nn.Linear(input_size, num_regression_outputs)
        num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        box_dim = len(box2box_transform.weights)
        self.bbox_pred = nn.Linear(input_size, num_bbox_reg_classes * box_dim)

        nn.init.normal_(self.cls_score.weight, std=0.01)
        nn.init.normal_(self.regressions.weight, std=0.01)
        nn.init.normal_(self.bbox_pred.weight, std=0.001)
        for l in [self.cls_score, self.regressions, self.bbox_pred]:
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "num_regression_outputs"    : cfg.MODEL.ROI_HEADS.NUM_REGRESSION_OUTPUTS,
            # fmt: on
        }
    ...
    def forward(self, x):
        ...
        regression_values = self.regressions(x)
        proposal_deltas = self.bbox_pred(x)
        return scores, regression_values, proposal_deltas
    ...
    def losses(self, predictions, proposals):
        ...
        """
        scores, regression_values, proposal_deltas = predictions
        ...
        gt_regressions = (
            cat([p.gt_regressions for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        )
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        regressions = self.predict_regressions(predictions, proposals)
    ...
    def predict_regressions(
        ...
        _, regressions, _ = predictions
        num_prop_per_image = [len(p) for p in proposals]
        return regressions.split(num_prop_per_image)